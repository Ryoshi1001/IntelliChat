  ...
  return (
    <div className="min-h-screen bg-primary relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div
          className="absolute bottom-10 right-10 w-96 h-96 bg-gradient-to-tl from-indigo-500/8 via-blue-600/4 to-transparent rounded-full blur-3xl animate-pulse"
      ...
      <div className="flex flex-col items-center justify-center h-screen p-4 sm:py-[4%] sm:px-[3%] relative z-10">
        ...
        <div className="max-sm:hidden absolute bottom-6 left-6 glass-card px-4 py-2 flex items-center gap-2 fade-in">
          <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
          <span className="text-xs text-secondary">Connected</span>
        </div>

        {/* Settings/Help button */}
        <div className="absolute bottom-6 right-6 flex gap-3 max-sm:hidden">
          <div className="relative text-[#fff] group">
            <button className="glass-card p-3 hover:glass-card-hover transition-all duration-300 rounded-full group cursor-pointer">
              <svg
            ...
            <div className="absolute hidden group-hover:block -top-20 right-4 bgdark py-3 px-4 z-10 rounded-sm">
              <div
                onClick={() => navigate("/profile")}
                className="hover:text-[var(--logoblue)] cursor-pointer text-nowrap"
              >
                Edit Profile
              </div>
              <hr className="my-1" />
              <div
                onClick={() => logout()}
                className="cursor-pointer hover:text-[var(--logoblue)]"
              >
                Logout
              </div>
            </div>
